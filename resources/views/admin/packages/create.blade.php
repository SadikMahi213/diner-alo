...
<div class='container mx-auto px-4 py-8'>
    <div class='flex justify-between items-center mb-6'>
        <h1 class='text-3xl font-bold text-gray-800'>Create Package</h1>
    ...
    <div class='bg-white shadow-md rounded-lg p-6'>
        <form action='{{ route('admin.packages.store') }}' method='POST'>
            ...
            <div class='mb-4'>
                ...
                <input type='number' name='price' id='price' step='0.01' min='0' value="{{ old('price') }}" class='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' required>
            </div>
            
            <div class='mb-4'>
                <label for='is_active' class='block text-gray-700 text-sm font-bold mb-2'>
                    <input type='checkbox' name='is_active' id='is_active' value="1" class='mr-2 leading-tight' {{ old('is_active', true) ? 'checked' : '' }}>
                    Is Active
                </label>
            </div>
                ...
                <div class='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    @foreach($courses as $course)
                        <div class='flex items-center'>
                            <input type='checkbox' name='courses[]' value='{{ $course->id }}' id='course_{{ $course->id }}' class='mr-2' {{ in_array($course->id, old('courses', [])) ? 'checked' : '' }}>
